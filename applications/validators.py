...
class SecureFileValidator:
    ...
    def __init__(self, allowed_extensions=None, max_size_mb=5):
        self.allowed_extensions = [ext.lower() for ext in (allowed_extensions or ['pdf', 'png', 'jpg', 'jpeg', 'doc', 'docx'])]
        ...
        self.mime_types = {
            ...
            'jpg': ['image/jpeg', 'image/jpg', 'image/jpe_', 'image/pjpeg', 'image/vnd.swiftview-jpeg', 'image/x-citrix-jpeg'],
            'jpeg': ['image/jpeg', 'image/jpg', 'image/jpe_', 'image/pjpeg', 'image/vnd.swiftview-jpeg', 'image/x-citrix-jpeg'],
    ...
    def __call__(self, file):
        ...
        try:
            ...
            raise ValidationError("Could not determine file type. The file might be corrupted.")

        expected_mimes = self.mime_types.get(ext, [])
        if mime_type not in expected_mimes:
            raise ValidationError(f"File content ({mime_type}) does not match the '{ext}' extension.")
